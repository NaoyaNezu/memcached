...
#ifdef FI //nezu added----------------------------------------------------------------
void show_items(void* address){
    item* it = (item*) address;
    unsigned long key_head = (unsigned long) ITEM_key(it);
    unsigned long key_tail = key_head + it->nkey;
    unsigned long val_head = (unsigned long) ITEM_data(it);
    unsigned long val_tail = val_head + it->nbytes;
    unsigned long meta_head = (unsigned long) it;
    unsigned long meta_tail = key_head - 1;

    if(it->it_flags & ITEM_LINKED){
        fprintf(stderr, "\tkey: %s => \n\t meta : 0x%lx ~ 0x%lx, key : 0x%lx ~ 0x%lx, val : 0x%lx ~ 0x%lx\n"\
                ,ITEM_key(it),meta_head,meta_tail,key_head,key_tail,val_head,val_tail);
    }
}


void check_lru_items(int state_id,int clsid){
    item *it;
    int index[3] = {HOT_LRU,WARM_LRU,COLD_LRU};
    char *state[3] = {"HOT","WARM","COLD"};
    int id = clsid + index[state_id];

    fprintf(stderr,"slabclss %d : %s List\n",clsid,state[state_id]);
    if(tails[id] != 0){
        it = heads[id];
        while(it != tails[id]){
            show_items((void*)it);
...
void check_lru_addr(int state_id,int clsid){
    unsigned long h_head = (unsigned long)heads;
    unsigned long h_tail = (unsigned long)heads + LARGEST_ID*sizeof(item*)-1;
    print_address("LRU_heads_List",h_head,h_tail,sizeof(item*),LARGEST_ID);

    unsigned long t_head = (unsigned long)tails;
    unsigned long t_tail = (unsigned long)tails + LARGEST_ID*sizeof(item*)-1;
    print_address("LRU_tails_List",t_head,t_tail,sizeof(item*),LARGEST_ID);

    fprintf(stderr,"--------------------------------------------------------------------------------------------------\n");
    check_lru_items(state_id,clsid);
    fprintf(stderr,"==================================================================================================\n");
}
#endif //-------------------------------------------------------------------------------------------------
...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    it->ecc_addr = make_item_ecc(it);
    fprintf(stderr,"make_item_ecc called\n");
#endif  //*----------------------------------------------------------------------------*
    return it;
}
...
#ifdef SOFT_ECC //nezu add-------------------------------------------------------------
    //update it->next's metadata ecc.
    if(it->ecc_addr->m_bch != NULL){ //if init process has been done.
        fprintf(stderr,"update_ecc from do_item_link_q,item:%s\n",ITEM_key(it));
        update_ecc(it,true,false,false);
        if(it->next){
            fprintf(stderr,"update_ecc call for next from do_item_link_q,item:%s\n",ITEM_key(it->next));
            update_ecc(it->next,true,false,false);
        }
    }
#endif //---------------------------------------------------------------------
...
#ifdef SOFT_ECC //nezu add-------------------------------------------------------------
    if(it->next && it->next->ecc_addr->m_bch != NULL){ //if init process has been done.
        fprintf(stderr,"update_ecc of next_item from do_item_unlink_q:%s\n",ITEM_key(it->next));
        update_ecc(it->next,true,false,false);
    }
    if(it->prev && it->prev->ecc_addr->m_bch != NULL){ //if init process has been done.
        fprintf(stderr,"update_ecc of prev_item from do_item_unlink_q:%s\n",ITEM_key(it->prev));
        update_ecc(it->prev,true,false,false);
    }
#endif 
...
#ifdef SOFT_ECC
    if(it->ecc_addr->m_bch != NULL){ //if init process has been done.
        fprintf(stderr,"update_ecc from do_item_link:%s\n",ITEM_key(it));
     update_ecc(it,true,false,false);
    }
#endif
...
#ifdef SOFT_ECC //nezu add-------------------------------------------------------
        fprintf(stderr,"free_ecc called from do_item_remove\n");
        free_ecc(it); //free ecc data structure
#endif  //------------------------------------------------------------
        item_free(it);
    }
}
...
#ifdef SOFT_ECC
            if(it->ecc_addr->m_bch != NULL){ //if init process has been done.
                fprintf(stderr,"update_ecc from do_item_update:%s\n",ITEM_key(it));
                update_ecc(it,true,false,false);
            }
#endif
                item_link_q_warm(it);
    ...
    if (settings.lru_segmented) {
        if ((it->it_flags & ITEM_ACTIVE) == 0) {
            if ((it->it_flags & ITEM_FETCHED) == 0) {
                fprintf(stderr,"ITEM_FETCHED\n");
                it->it_flags |= ITEM_FETCHED;
            } else {
                it->it_flags |= ITEM_ACTIVE;
#ifdef SOFT_ECC
                if(it->ecc_addr->m_bch != NULL){ //if init process has been done.
                    fprintf(stderr,"update_ecc from do_item_bump:%s\n",ITEM_key(it));
                    update_ecc(it,true,false,false);
                }
#endif
...
item *do_item_touch(const char *key, size_t nkey, uint32_t exptime,
    ...
    if (it != NULL) {
        it->exptime = exptime;
#ifdef SOFT_ECC //nezu add---------------------------------------------------------
        fprintf(stderr,"update_ecc is called from do_item_touch:%s\n",ITEM_key(it));
        update_ecc(it,true,false,false);
#endif  //-----------------------------------------------------------------
    }
    return it;
...
int lru_pull_tail(const int orig_id, const int cur_lru,
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        ...
        //fprintf(stderr,"incr from lru_pull tail\n");
        if (refcount_incr(search) != 2) {
            ...
            if (settings.tail_repair_time &&
                ...
                search->refcount = 1;
#ifdef SOFT_ECC
            if(search->ecc_addr->m_bch != NULL){ //if init process has been done.
                fprintf(stderr,"update_ecc from lru_pull_tail:%s\n",ITEM_key(search));
                update_ecc(search,true,false,false);
            }
#endif
...
#ifdef SOFT_ECC
                    if(search->ecc_addr->m_bch != NULL){ //if init process has been done.
                        fprintf(stderr,"update_ecc from lru_pull_tail WARM case:%s\n",ITEM_key(search));
                        update_ecc(search,true,false,false);
                    }
#endif
                    if (cur_lru == WARM_LRU) {
...
#ifdef SOFT_ECC
                if(search->ecc_addr->m_bch != NULL){ //if init process has been done.
                    fprintf(stderr,"update_ecc from lru_pull_tail COLD case:%s\n",ITEM_key(search));
                    update_ecc(search,true,false,false);
                }
#endif
                    do_item_unlink_q(search);
    ...
    if (it != NULL) {
        ...
        if ((flags & LRU_PULL_RETURN_ITEM) == 0) {
            //fprintf(stderr,"LRU_PULL_TAIL\n")
            //fprintf(stderr,"decr from lru_pull tail\n");
            do_item_remove(it);
            item_trylock_unlock(hold_lock);
        }
    }
...
item *do_item_crawl_q(item *it) {
    ...
    if (it->prev == 0) {
        ...
        if (it->next) {
            *head = it->next;
            ...
            it->next->prev = 0;
#ifdef SOFT_ECC
        if(it->next->ecc_addr->m_bch != NULL){ //if init process has been done.
            fprintf(stderr,"update_ecc from do_item_crawl_q 0:%s\n",ITEM_key(it->next));
            // item *hoge = it->next;
            // fprintf(stderr,"\tcq_0:%p,%p,%p,%d,%d,%d,%d,%d,%d\n",(void*)hoge->next,(void*)hoge->prev,(void*)hoge->h_next,hoge->time,hoge->exptime,hoge->nbytes,hoge->refcount,hoge->it_flags,hoge->slabs_clsid);
            update_ecc(it->next,true,false,false);
        }
#endif
        }
    ...
    if (it->prev) {
...
#ifdef SOFT_ECC
        if(it->next->ecc_addr->m_bch != NULL){ //if init process has been done.
            fprintf(stderr,"update_ecc from do_item_crawl_q 1:%s\n",ITEM_key(it->next));
            // item *hoge = it->next;
            // fprintf(stderr,"\tcq_1:%p,%p,%p,%d,%d,%d,%d,%d,%d\n",(void*)hoge->next,(void*)hoge->prev,(void*)hoge->h_next,hoge->time,hoge->exptime,hoge->nbytes,hoge->refcount,hoge->it_flags,hoge->slabs_clsid);
            update_ecc(it->next,true,false,false);
        }
#endif
        } else {
...
#ifdef SOFT_ECC
        if(it->next->ecc_addr->m_bch != NULL){ //if init process has been done.
            fprintf(stderr,"update_ecc from do_item_crawl_q 2:%s\n",ITEM_key(it->next));
            // item *hoge = it->next;
            // fprintf(stderr,"\tcq_2:%p,%p,%p,%d,%d,%d,%d,%d,%d\n",(void*)hoge->next,(void*)hoge->prev,(void*)hoge->h_next,hoge->time,hoge->exptime,hoge->nbytes,hoge->refcount,hoge->it_flags,hoge->slabs_clsid);
            update_ecc(it->next,true,false,false);
        }
#endif
        ...
        if (it->prev) {
            it->prev->next = it;
#ifdef SOFT_ECC
        if(it->prev->ecc_addr->m_bch != NULL){ //if init process has been done.
            fprintf(stderr,"update_ecc from do_item_crawl_q 3:%s\n",ITEM_key(it->next));
            // item *hoge = it->prev;
            // fprintf(stderr,"\tcq_3:%p,%p,%p,%d,%d,%d,%d,%d,%d\n",(void*)hoge->next,(void*)hoge->prev,(void*)hoge->h_next,hoge->time,hoge->exptime,hoge->nbytes,hoge->refcount,hoge->it_flags,hoge->slabs_clsid);
            update_ecc(it->prev,true,false,false);
        }
#endif
        }
