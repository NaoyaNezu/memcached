...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas, bool should_touch) {
    ...
    size_t nkey;
    item *it;
    token_t *key_token = &tokens[KEY_TOKEN]; //キー部分のトークンをkey_tokenに格納
    ...
    if (should_touch) { //おそらくエラー処理だからここには入らない
        ...
        if (of.set_stale) {
            ...
            it->it_flags &= ~ITEM_TOKEN_SENT;
                ...
                if(it->ecc_addr->m_bch != NULL){ //if init process has been done.
                    fprintf(stderr,"update_ecc from item_get_lock:%s\n",ITEM_key(it));
...
#ifdef FI
//nezu add----------------------------------------------------------------------------
static void process_show_slab_command(conn *c,token_t *tokens){
    int clsid;
    int slabid;
    if (! (safe_strtol(tokens[1].value, (int32_t *)&clsid)
        && safe_strtol(tokens[2].value, (int32_t *)&slabid))) {
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    if(check_slabclass_array_addr(c,clsid,slabid) != -1){
        out_string(c,"show_slab_addr received");
    }
}

static void process_show_hash_command(conn *c,token_t *tokens){
    int h_id;
    int t_id;
    if (! (safe_strtol(tokens[1].value, (int32_t *)&h_id)
        && safe_strtol(tokens[2].value, (int32_t *)&t_id))) {
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    if(check_hashtbl_addr(c,h_id,t_id)!= -1){
        out_string(c,"show_hash_addr received");
    }
}

static void process_show_lru_command(conn *c,token_t *tokens){
    char* state;
    int state_id;
    int clsid;
    char errmsg[256];
    state = tokens[1].value;
    sprintf(errmsg,"ERROR : arg 3 needs to be 0 ~ %d",MAX_NUMBER_OF_SLAB_CLASSES-1);

    if(!(strcmp(state,"HOT"))){
        state_id = 0;
    } else if(!(strcmp(state,"WARM"))){
        state_id = 1;
    } else if(!(strcmp(state,"COLD"))){
        state_id = 2;
    } else{
        out_string(c,"ERROR : arg 2 need to be HOT or WARM or COLD");
        return;
    }

    if (! (safe_strtol(tokens[2].value, (int32_t *)&clsid))) {
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    
    if(clsid < 0 || clsid > MAX_NUMBER_OF_SLAB_CLASSES-1){
        out_string(c,errmsg);
        return;
    }
    check_lru_addr(state_id,clsid);
    out_string(c,"show_lru_addr received");
}

static void process_fault_inject_command(conn *c,token_t *tokens){
    char *addr;
    ...
    addr_num = strtol(addr,&endptr,16);
    if(strcmp(endptr,"")){
        out_string(c,"ERROR : arg 1 needs to be Hexadecimal");
        return;
    }
    p = (void*) addr_num;

    if (! (safe_strtol(tokens[2].value, (int32_t *)&bit))) {
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    if (bit < 1 || bit > 8){
        out_string(c,"ERROR : arg 2 needs to be 1 ~ 8");
        return;
    }

    fault_inject(p,bit);
    out_string(c,"inject received");
...
static void process_command(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        #ifdef FI
        else if(strcmp(tokens[COMMAND_TOKEN].value, "show_slab_addr") == 0){
            WANT_TOKENS_OR(ntokens, 4, 4);
            process_show_slab_command(c,tokens);
        } else if(strcmp(tokens[COMMAND_TOKEN].value, "show_hash_addr") == 0){
            WANT_TOKENS_OR(ntokens, 4, 4);
            process_show_hash_command(c,tokens);
        } else if(strcmp(tokens[COMMAND_TOKEN].value, "show_lru_addr") == 0){
            WANT_TOKENS_OR(ntokens, 4, 4);
            process_show_lru_command(c,tokens);
        }
        #endif
        else {
            out_string(c, "ERROR");
        ...
        #ifdef FI
         else if(strcmp(tokens[COMMAND_TOKEN].value, "inject") == 0){
            WANT_TOKENS_OR(ntokens, 4, 4);
            process_fault_inject_command(c,tokens);
        } 
        #endif
        else {
            out_string(c, "ERROR");
