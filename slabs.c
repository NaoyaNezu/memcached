...
#ifdef FI
//nezu added------------------------------------------------------------------
//print out the information of slab class like -vvv option


// void process_show_lru_command(conn*c){
//     int i = 0;
//     slabclass_t *p;

//     while(++i < MAX_NUMBER_OF_SLAB_CLASSES){
//         p = &slabclass[i];
//         fprintf(stderr,"slab class %3d: chunk size %9u perslab %7u slabs %7u\n",
//         i,p->size,p->perslab,p->slabs);
//         if(p->slabs > 0){
//             lru_show(i);
//         }
//     }
...
//for slabclass show
void search_items(int clsid,void* slab_p){
    slabclass_t *class_p = &slabclass[clsid];
    unsigned int size = class_p->size;
    unsigned int perslab = class_p->perslab;
    unsigned long slab_head = (unsigned long) slab_p;
    unsigned long item_p = slab_head + size*(perslab-1);
    void * p;
...
void check_slabs_item(int clsid,int slabid){
    slabclass_t *class_p = &slabclass[clsid];
    int slabs = class_p->slabs;
    void *slab_p;
    unsigned int slab_size = settings.slab_page_size;

    if(slabid > slabs){
        fprintf(stderr,"thre is not %d slabs. slab class %d has %d slabs.",slabid,clsid,slabs);
        return;
    }
    slab_p = class_p->slab_list[slabid-1];
    fprintf(stderr,"slab%d : 0x%lx ~ 0x%lx\n",slabid,(unsigned long)slab_p,(unsigned long)slab_p+slab_size-1);
    search_items(clsid,slab_p);
}

void check_slabs_inside(int clsid,int slabid){
    slabclass_t *p;
    p = &slabclass[clsid];
    fprintf(stderr,"slab class %3d: chunk size => %5u perslab => %5u slabs =>%3u\n",
            clsid,p->size,p->perslab,p->slabs);
    if(p->slabs > 0){
        fprintf(stderr,"--------------------------------------------------------------------------------------------------\n");
        check_slabs_item(clsid,slabid);
    }
}

int check_slabclass_array_addr(conn* c,int clsid,int slabid){
    char errmsg[256];
    sprintf(errmsg,"ERROR : slabclassID needs to be 0 ~ %d",MAX_NUMBER_OF_SLAB_CLASSES-1);
    if(clsid < 0 || clsid > MAX_NUMBER_OF_SLAB_CLASSES-1){
        out_string(c,errmsg);
        return -1;
    }
    unsigned long head = (unsigned long)slabclass;
    unsigned long tail = (unsigned long)slabclass + MAX_NUMBER_OF_SLAB_CLASSES*sizeof(slabclass_t)-1;
    print_address("slabclass_array",head,tail,sizeof(slabclass_t),MAX_NUMBER_OF_SLAB_CLASSES);
    check_slabs_inside(clsid,slabid);
    return 0;
}

//一旦スラブクラス［０］のメモリプールがどうなってるかを調べるために作った仮のやつ
void check_slab(){
    slabclass_t *p = &slabclass[SLAB_GLOBAL_PAGE_POOL];
    void ** t = p->slab_list;
    fprintf(stderr,"%d\n",p->slabs);
    for (int i = 0;i < p->slabs;i++){
        fprintf(stderr,"%p\n",*t);
        t++;
    }
}
