...
void check_hash_items(int h_id,int t_id){
    item* it;
    item **p = &primary_hashtable[h_id];
    for (int i = h_id;i <= t_id;i++){
        if(*p != NULL){
...
int check_hashtbl_addr(conn* c,int h_id,int t_id){
    int table_size = hashsize(hashpower);
    char errmsg[256];
    unsigned long head = (unsigned long)primary_hashtable;
    unsigned long tail = (unsigned long)primary_hashtable + table_size*sizeof(item**)-1;
    sprintf(errmsg,"ERROR : Table range need to be 0 ~ %d",table_size-1);

    if(h_id < 0 || t_id > table_size-1 || h_id > t_id){
        out_string(c,errmsg);
        return -1;
    }
    print_address("Hash_table",head,tail,sizeof(item**),table_size);
    fprintf(stderr,"--------------------------------------------------------------------------------------------------\n");
    check_hash_items(h_id,t_id);
    fprintf(stderr,"==================================================================================================\n");
    return 0;
}
#endif
...
#ifdef SOFT_ECC
        //nezu add for hash research ----------------------------------------------------------------
        fprintf(stderr,"check_ecc is called from assoc_find\n");
        //--------------------------------------------------------------------------------------
        check_ecc(it);
        ...
        (oldbucket = (hv & hashmask(hashpower - 1))) >= expand_bucket)
    {
        ...
        //nezu add--------------------------------------------------------------------------------
         if(it->ecc_addr->m_bch != NULL){
            ...
            fprintf(stderr,"update_ecc called from assoc_insert;%s\n",ITEM_key(it));
        }//---------------------------------------------------------------------------------------
#endif
    } else {
...
static void *assoc_maintenance_thread(void *arg) {
    ...
    while (do_run_maintenance_thread) {
        fprintf(stderr,"assoc_maintenance\n");
        int ii = 0;
        ...
        for (ii = 0; ii < hash_bulk_move && expanding; ++ii) {
            ...
            if ((item_lock = item_trylock(expand_bucket))) {
                    for (it = old_hashtable[expand_bucket]; NULL != it; it = next) {
                        ...
                        if(it->ecc_addr->m_bch != NULL){
                            update_ecc(it,true,false,false);
                            fprintf(stderr,"update_ecc called from assoc_maintenance_thread:%s",ITEM_key(it));
                        }//---------------------------------------------------------------------------------------
#endif
